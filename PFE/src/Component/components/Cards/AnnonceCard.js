...
export default function AnnonceCard({ annonce , categories , images}) {
  const { id, titre, description, prix, image, surface, id_categorie, adresse, statut } = annonce;
  ...
  return (
    <div className=" bg-slate-50 shadow-lg rounded-lg   ml-20
    rounded-tl-[98px] w-full mx-auto my-7 max-w-[298px] max-h-[530px] cursor-pointer hover:shadow-3xl transition
     relative  transform hover:scale-105  hover:bg-gradient-to-r from-slate-200 to-blue-50   overflow-visible hover:shadow-blue-300">
      {annonceImages.length > 0 && (
        <img src={annonceImages} alt={titre} className="w-full rounded-ss-lg rounded-t-lg h-44 object-cover" />
      )}
